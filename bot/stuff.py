...
async def coding(event):
    try:
        text = event.raw_text.split(" ", 1)[1]
        ffmpegcode[0] = text
        await event.reply(f"✅ FFMPEG code updated to:\n\n`{text}`")
    except IndexError:
        await event.reply("⚠️ Please provide ffmpeg code.\nUsage:\n/setcode -preset fast -crf 28 -c:v libx264")


async def getcode(event):
    await event.reply(f"🎯 Current ffmpeg code:\n\n`{ffmpegcode[0]}`")
